...
export default function Contact() {
  return (
    <section id="contact" className="mx-auto max-w-6xl px-6 py-24 sm:px-10">
      <Reveal className="gradient-border rounded-3xl p-px">
        <div className="bg-noise overflow-hidden rounded-[calc(1.5rem-1px)] bg-ink text-white">
          <div className="grid gap-12 p-10 sm:p-14 md:grid-cols-2">
            <div>
              ...
              <a
                ...
                download
                className="mt-8 inline-flex items-center gap-2 rounded-full bg-brand px-6 py-3 text-sm font-semibold text-white transition-all hover:bg-brand-light hover:shadow-[0_0_30px_-5px_var(--brand-light)]"
              >
            ...
            <div className="space-y-4">
              {CONTACT_DETAILS.map(({ icon: Icon, label, value, href }) => {
                const content = (
                  <div className="flex items-center gap-4 rounded-2xl border border-white/10 bg-white/5 p-5 transition-all hover:border-brand-light/40 hover:bg-white/10">
          ...
          <div className="border-t border-white/10 p-10 sm:p-14">
            ...
            <div className="mt-4 grid gap-4 sm:grid-cols-2">
              {REFERENCES.map((ref) => (
                <div
                  key={ref.name}
                  className="rounded-xl border border-white/10 bg-white/5 px-5 py-4 transition-colors hover:border-brand-light/40"
                >
